...
ht_agg_pandas_df['labeled_lifestyle'] = y

# COMMAND ----------

ht_agg_pandas_df[['lifestyle', 'labeled_lifestyle']].drop_duplicates()

# COMMAND ----------

dir(ht_agg_pandas_df)
...
# make distinct labels
label_y_train = np.unique(y_train_1_predicted)
label_y_test = np.unique(y_test_1_predicted)
...
print(confusion_matrix(y_train, y_train_1_predicted, labels=[0,1,2,3]))
print("")
...
# make confusion matrix into a dataframe
import numpy as np
import pandas as pd
...
# get output confusion matrices
cmTrain = confusion_matrix(y_train, y_train_1_predicted)
cmTest = confusion_matrix(y_test, y_test_1_predicted)


# .DataFrame()
pd.DataFrame(cmTrain, index=labels, columns=labels)
